					 ...
					 const struct bgp_path_info *rmap_bpi)
{
	...
	/* Type 5 route export */
	if (advertise_type5_vpn_routes(to_bgp, afi) && is_route_injectable_into_evpn(new))
				...
				struct bgp_path_info *path_vrf) /* route */
{
	...
	while (bpi_next) {
		...
		if (advertise_type5_vpn_routes(to_bgp, afi) && is_route_injectable_into_evpn(bpi))
			bgp_evpn_unexport_type5_route_rd(to_bgp, bn, bpi, afi, safi,
							 &(from_bgp->vpn_policy[afi].tovpn_rd));
		bgp_aggregate_decrement(to_bgp, p, bpi, afi, safi);
		bgp_path_info_mark_for_delete(bn, bpi);
		bgp_process(to_bgp, bn, bpi, afi, safi);
		...
		for (bn = bgp_table_top(table); bn; bn = bgp_route_next(bn)) {
			...
			for (; (bpi != NULL) && (next = bpi->next, 1);
				...
				if ((struct bgp *)bpi->extra->vrfleak->bgp_orig ==
					...
					vpn_leak_to_vrf_withdraw(bpi);
					if (advertise_type5_vpn_routes(to_bgp, afi) &&
