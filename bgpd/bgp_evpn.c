...
static void delete_withdraw_vrf_routes(struct bgp *bgp_vrf)
{
	...
	if (advertise_type5_vpn_routes(bgp_vrf, AFI_IP))
		bgp_evpn_withdraw_type5_routes(bgp_vrf, AFI_IP, SAFI_MPLS_VPN);

	if (advertise_type5_vpn_routes(bgp_vrf, AFI_IP6))
		...
		for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
			...
			*/
			for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
				...
				if (safi == SAFI_MPLS_VPN)
					bgp_evpn_withdraw_type5_route_vpn(bgp_vrf, pi,
									  bgp_dest_get_prefix(dest),
									  afi, safi, addpath_id);
				else
			...
			for (bn = bgp_table_top(table); bn; bn = bgp_route_next(bn)) {
				for (bpi = bgp_dest_get_bgp_path_info(bn); bpi; bpi = bpi->next) {
					if (!is_route_injectable_into_evpn(bpi))
						continue;
					addpath_id = bgp_evpn_addpath_id_for_path(bgp_vrf, bpi,
										  afi);
